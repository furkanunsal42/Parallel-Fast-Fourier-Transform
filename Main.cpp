...
int main() {
	std::vector<OMP_FFT::complex> signal = OMP_FFT::generate_cos_signal(1024 * 64, 2, 4, OMP_FFT::PI / 2);
	std::vector<OMP_FFT::complex> signal_copy = signal;
	...
	OMP_FFT::parallel_fft_radix2(signal);
	...
	OMP_FFT::parallel_inverse_fft_radix2(signal);
